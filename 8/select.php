...
if($status==false){
  ...
  while( $result = $stmt->fetch(PDO::FETCH_ASSOC)){
    ...
    $view .= '<span class="name">'.h($result["name"]).'</span>'.'<span class="datetime">'."[".h($result["indate"]).'</span>'."]";
    $view .= '</a>　';
...
<style>
    ...
    span.name{background:#000;display:inline-block;color:#FFF;padding:10px;width:200px;text-align:center;margin-left:20px;}
